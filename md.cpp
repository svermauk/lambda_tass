...
void gmx::Integrator::do_md()
{
   ...
   ********************************************************************/
    ...
    double running_lambda = state->lambda[0], bias_k0=10.0, bias_k1=25.0, force=0.0, mass=29.0;
        ...
        if (ir->efep != efepNO && !EI_VV(ir->eI))
        {
         ...
         // $$$$$$$$$$$$$$$$$$$$$$$$$$   Calculating Force @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ -- done by mj 
           //if (running_lambda < min_lambda )
            //{
             force =-enerd->term[F_DVDL]- bias_k0*(running_lambda - min_lambda );
             enerd->term[F_EPOT]+=0.5*bias_k0*(running_lambda - min_lambda)*(running_lambda - min_lambda);
             enerd->term[F_EKIN]+=0.5*mass*(running_v_lambda)*(running_v_lambda);
             // }
            //else if (running_lambda > max_lambda )
           // {
           //force = -enerd->term[F_DVDL] - bias_k1*(running_lambda - max_lambda);
...
//            printf("%f\t%f\t%f\t%f\t\n",t , enerd->term[F_EPOT], enerd->term[F_EKIN], enerd->term[F_ETOT]);            
   printf("%f\t%f\t%f\t%f\t%f\t%f\t\n", t , running_lambda, running_v_lambda, enerd->term[F_EPOT], enerd->term[F_EKIN], enerd->term[F_ETOT]);
